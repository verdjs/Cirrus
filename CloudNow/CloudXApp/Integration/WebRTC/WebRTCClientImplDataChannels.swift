...
extension WebRTCClientImpl {
    ...
    nonisolated func markDataChannelOpened(_ dataChannel: RTCDataChannel) {
        if let kind = callbackStateBox.activeDataChannelContext(for: dataChannel)?.kind {
            stateBox.markDataChannelOpened(kind)
        }
    }

    nonisolated func activeDataChannelContext(for dataChannel: RTCDataChannel) -> (generation: UInt64, kind: DataChannelKind)? {
        callbackStateBox.activeDataChannelContext(for: dataChannel)
    }

    nonisolated func callbackDelegate(for generation: UInt64) -> (any WebRTCBridgeDelegate)? {
