...
extension WebRTCClientImpl: @preconcurrency RTCAudioSessionDelegate {
    nonisolated func scheduleTVOSAudioReconcile(reason: String) {
    ...
    nonisolated func reconcileTVOSAudioSession(reason: String) {
    ...
    public nonisolated func audioSessionDidStartPlayOrRecord(_ session: RTCAudioSession) {
    ...
    public nonisolated func audioSessionDidStopPlayOrRecord(_ session: RTCAudioSession) {
        logAudio("didStopPlayOrRecord active=\(session.isActive)")
    }

    public nonisolated func audioSession(_ session: RTCAudioSession, didChangeCanPlayOrRecord canPlayOrRecord: Bool) {
        logAudio("canPlayOrRecord=\(canPlayOrRecord)")
    }

    public nonisolated func audioSession(_ audioSession: RTCAudioSession, willSetActive active: Bool) {
        logAudio("willSetActive=\(active)")
    }

    public nonisolated func audioSession(_ audioSession: RTCAudioSession, didSetActive active: Bool) {
    ...
    public nonisolated func audioSession(_ audioSession: RTCAudioSession, failedToSetActive active: Bool, error: Error) {
        logAudio("failedToSetActive=\(active) error=\(error.localizedDescription)", force: true)
    }

    public nonisolated func audioSession(_ audioSession: RTCAudioSession, audioUnitStartFailedWithError error: Error) {
        logAudio("audioUnitStartFailed error=\(error.localizedDescription)", force: true)
    }

    public nonisolated func audioSessionDidChangeRoute(_ session: RTCAudioSession,
    ...
    public nonisolated func audioSession(_ audioSession: RTCAudioSession, didChangeOutputVolume outputVolume: Float) {
