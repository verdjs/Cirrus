...
extension WebRTCClientImpl {
    nonisolated func peerConnectionGeneration(for peerConnection: RTCPeerConnection) -> UInt64? {
        callbackStateBox.activePeerConnectionGeneration(for: peerConnection)
    }

    nonisolated func appendLocalCandidate(_ payload: IceCandidatePayload) {
        stateBox.appendLocalCandidate(payload)
    }

    nonisolated func handlePeerConnectionConnected(generation: UInt64) {
#if os(tvOS)
    ...
    nonisolated func handlePeerConnectionTrackStart(
    ...
    nonisolated func publishKnownRemoteTracks(reason: String) {
    ...
    nonisolated func publishRemoteTrackIfNeeded(
        _ track: RTCMediaStreamTrack,
        reason: String,
        generation: UInt64
    ) {
