...
struct CloudXStreamView: View {
    ...
    @State private var bitrateHistory: [Double] = []
    ...
    var body: some View {
        ...
        .task(id: sessionIdentity) {
            guard let session = streamController.streamingSession else {
                pingHistory = []
                fpsHistory = []
                bitrateHistory = []
                return
            ...
            while !Task.isCancelled {
                let rtt = session.stats.roundTripTimeMs ?? 0.0
                let fps = session.stats.framesPerSecond ?? 0.0
                let bitrate = Double(session.stats.bitrateKbps ?? 0) / 1000.0
                
                if pingHistory.count >= 30 { pingHistory.removeFirst() }
                pingHistory.append(rtt)
                
                if fpsHistory.count >= 30 { fpsHistory.removeFirst() }
                fpsHistory.append(fps)
                
                if bitrateHistory.count >= 30 { bitrateHistory.removeFirst() }
        ...
        .onExitCommand {
            // tvOS may route the Siri Remote Menu button here. Treat it like Play/Pause:
            // it opens or closes stream controls, never exits the stream directly.
            Task {
                await streamController.setOverlayVisible(!overlayVisible, trigger: .userToggle)
            }
        }
...
private struct StreamViewPreviewHost: View {
    ...
    var body: some View {
        CloudXStreamView(context: .cloud(titleId: TitleID("preview-title-id")))
...
#Preview("CloudXStreamView", traits: .fixedLayout(width: 1920, height: 1080)) {
