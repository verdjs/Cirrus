...
struct CloudLibraryView: View {
        ...
        .fullScreenCover(item: $activeStreamContext, onDismiss: handleActiveStreamDismissed) { ctx in
            ZStack {
                ...
                StreamControllerInputHost(onOverlayToggle: {
                    ...
                    if streamController.isStreamOverlayVisible {
                        streamController.requestDisconnect()
                    }
                }) {
                    CloudXStreamView(context: ctx)
                }
            }
            .ignoresSafeArea()
    ...
    private var mountedShell: some View {
        ...
        .onChange(of: gamesViewModel.libraryGames, initial: true) { _, games in
            // Sync normalized GFN titles into the view model so the library badge
            // count can include GFN-only games (deduped against Xbox titles).
            vm.gfnLibraryTitles = Set(games.map {
                $0.title.lowercased()
    ...
    private func handleActiveStreamDismissed() {
        Task { @MainActor in
            ...
            await actionCoordinator.handleStreamDismiss(
                returnHome: { routeState.returnHome() },
