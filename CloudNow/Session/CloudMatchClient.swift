...
actor CloudMatchClient {
    ...
    private func toSessionInfo(base: String, payload: CloudMatchResponse, rawData: Data, clientId: String, deviceId: String) throws -> SessionInfo {
        ...
        // Always dump the full raw session JSON so we can see exactly which fields
        // GFN sends during the queue/preparing phase (e.g. real queue position key name).
        var rawDebugLog: String? = nil
        if let root = try? JSONSerialization.jsonObject(with: rawData) as? [String: Any],
           let sess = root["session"] as? [String: Any] {
            // Strip large/noisy fields before printing so the log stays readable
            var debugSess = sess
            debugSess.removeValue(forKey: "iceServerConfiguration")
            debugSess.removeValue(forKey: "connectionInfo")
            if let debugData = try? JSONSerialization.data(withJSONObject: debugSess, options: .prettyPrinted),
               let debugStr = String(data: debugData, encoding: .utf8) {
                print("[CloudMatch] rawSession (status=\(s.status)):\n\(debugStr)")
                rawDebugLog = debugStr
            }
        }
        ...
        return SessionInfo(
            ...
            signalingUrl: signalingUrl,
            gpuType: s.gpuType,
            queuePosition: s.seatSetupInfo?.queuePosition,
            seatSetupStep: s.seatSetupInfo?.seatSetupStep,
            iceServers: iceServers,
            mediaConnectionInfo: media,
            clientId: clientId,
            deviceId: deviceId,
            adState: adState,
            rawDebugLog: rawDebugLog
        )
    }
