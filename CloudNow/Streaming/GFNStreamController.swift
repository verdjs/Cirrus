...
final class GFNStreamController: NSObject {
    private(set) var state: GFNStreamState = .idle
    ...
    /// Callback fired when controller Left + Right bumpers are pressed.
    public var onStatsToggle: (() -> Void)?
    ...
    func sendVirtualKeyEvent(down: Bool, vk: UInt16, scancode: UInt16, modifiers: UInt16) {
        inputSender?.sendVirtualKeyEvent(down: down, vk: vk, scancode: scancode, modifiers: modifiers)
    }
        ...
        do {
            let answer: LKRTCSessionDescription = try await withCheckedThrowingContinuation { cont in
                pc.answer(for: answerConstraints) { sdp, error in
                    ...
                    if let sdp { cont.resume(returning: sdp) } else { cont.resume(throwing: GFNStreamError.noSDP) }
                }
            }
    ...
    private func attachMicrophone(to pc: LKRTCPeerConnection) async {
        #if os(tvOS)
        // Record permission is not required/available on tvOS in this manner
        #else
    ...
    private func addRemoteICE(candidate: String, sdpMid: String?, sdpMLineIndex: Int?) {
        ...
        Task {
            do {
                ...
                print("[ICE] Failed to add remote candidate: \(error)")
            }
        }
    }
...
extension GFNStreamController: LKRTCPeerConnectionDelegate {
    ...
    nonisolated func peerConnection(_ peerConnection: LKRTCPeerConnection, didOpen dataChannel: LKRTCDataChannel) {
        ...
        if dataChannel.label == "control_channel" || dataChannel.label == "input_channel_v1" || dataChannel.label == "input_channel_partially_reliable" {
            dataChannel.delegate = self
            Task { @MainActor [weak self] in
                guard let self else { return }
                if dataChannel.label == "control_channel" {
                    self.controlChannel = dataChannel
                } else if dataChannel.label == "input_channel_v1" {
                    self.inputDataChannel = dataChannel
                    print("[DataChannel] Bound to server-opened inputDataChannel")
                } else if dataChannel.label == "input_channel_partially_reliable" {
                    self.partiallyReliableDataChannel = dataChannel
                    print("[DataChannel] Bound to server-opened partiallyReliableDataChannel")
                }
            }
        }
    }
...
extension GFNStreamController: LKRTCDataChannelDelegate {
    ...
    nonisolated func dataChannel(_ dataChannel: LKRTCDataChannel, didReceiveMessageWith buffer: LKRTCDataBuffer) {
        ...
        if firstWord == 526 {
            ...
            let hexString = bytes.prefix(18).map { String(format: "%02X", $0) }.joined(separator: " ")
            print("[DataChannel] Non-handshake message on \(dataChannel.label): count=\(bytes.count), prefix=[\(hexString)]")

            Task { @MainActor [weak self] in
                self?.inputSender?.handleIncomingMessage(bytes)
            }
            return
        }
        ...
        Task { @MainActor [weak self] in
            ...
            sender.vibrationEnabled = self.settings.vibrationEnabled
            ...
            sender.statsToggleHandler = { [weak self] in self?.onStatsToggle?() }
