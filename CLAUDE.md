...
- `NVIDIAAuthAPI.swift` — Raw federated NVIDIA/cloud.gg OAuth endpoints: device authorization, token exchange, refresh, client_token rebinding.
...
- `LoginView.swift` — Displays a QR code and PIN for federated cloud.gg device flow login; user scans the QR code or visits the URL on any device to complete OAuth.

## Key Patterns

- **State**: `@Observable + @MainActor` throughout (AuthManager, GFNStreamController, GamesViewModel). No Combine/Redux.
- **Auth flow**: cloud.gg device flow via federated NVIDIA gateway (TV shows QR code + PIN; user completes on any device) → token stored in Keychain → silent refresh on launch → `client_token` rebind for GraphQL.
